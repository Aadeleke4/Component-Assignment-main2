...
describe('Dropdown Component', () => {
  ...
  it('should be visible when rendered', () => {
    render(<Dropdown label="Drop Down" options={["Option 1", "Option 2", "Option 3"]} onSelect={() => {}} />);
  ...
  it('should be disabled when the disabled prop is true', () => {
    render(<Dropdown label="Disabled Drop Down" options={["Option 1", "Option 2", "Option 3"]} disabled={true} />);
    const dropdownButton = screen.getByRole('button', { name: /select/i });
  ...
  it('should change background color when disabled', () => {
    const { rerender } = render(<Dropdown label="Drop Down" options={["Option 1", "Option 2", "Option 3"]} />);
    const dropdownButton = screen.getByRole('button', { name: /select/i });
    expect(dropdownButton).not.toHaveStyle('background-color: grey'); // Assuming default is not grey

    rerender(<Dropdown label="Disabled Drop Down" options={["Option 1", "Option 2", "Option 3"]} disabled={true} backgroundColor="grey" />);
